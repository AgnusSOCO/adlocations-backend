...
export async function estimateAdPrice(params: {
  ...
  material?: string;
  notes?: string;
}): Promise<{ priceEstimate: number; reasoning: string }> {
    ...
    messages: [
      {
        role: "system",
        content:
          "You are an expert in outdoor advertising pricing. Provide realistic price estimates based on location, size, and market conditions.",
      },
    ...
    messages: [
      {
        role: "system",
        content:
          "You are a professional advertising sales representative. Generate compelling, professional quotes for outdoor advertising spaces.",
      },
      { role: "user", content: prompt },
    ],
  });
...
export async function predictRenewalLikelihood(params: {
  ...
  rentAmount: number;
}): Promise<{ likelihood: number; reasoning: string; recommendations: string[] }> {
  const monthsRemaining = Math.max(
    0,
    Math.floor(
      (params.contractEndDate.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24 * 30)
    )
  );

  const contractDuration = Math.floor(
    (params.contractEndDate.getTime() - params.contractStartDate.getTime()) /
      (1000 * 60 * 60 * 24 * 30)
  );
    ...
    messages: [
      {
        role: "system",
        content:
          "You are a customer success analyst specializing in advertising contracts. Provide data-driven renewal predictions.",
      },
      { role: "user", content: prompt },
    ],
    response_format: {
